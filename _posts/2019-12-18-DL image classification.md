---
layout: post
title: "AWS를 활용한 딥러닝 서비스 Severless API 개발 - Part 1. Amazon SageMaker에서 PyTorch를 활용한 딥러닝 이미지 분류 모델 학습 및 배포"
...
# AWS를 활용한 딥러닝 서비스 Severless API 개발 (Part 1)
 ...
 안녕하세요. 어반베이스 Machine Learning 개발자 백윤아입니다. 어반베이스에서는 공간 분석 서비스인 Space 개발을 담당하고 있습니다.

 Space는 공간분석 AI 서비스로 크게 공간 분류 / 사물 검출 / 인테리어 스타일 분석 / 제품 추천 이렇게 4가지 기능들이 포함되어 있습니다. 저는 공간 이미지 데이터를 가공 및 전처리해서 공간을 분석하고, 분석한 결과를 바탕으로 개인에게 맞춤화된 추천 시스템 알고리즘을 설계하는 일을 하고 있습니다.


머지않아 외부 업체 최초로 SK Open API 포털에 저희 Space AI API가 등록될 예정입니다. 대부분의 CV 기술이 얼굴이나 사물 인식 쪽에 치우쳐 있는데, Space AI는 공간을 기반으로 하다보니 커머스 쪽에서 활용도가 높을 듯 합니다. 

 이번 글에서는 Space AI를 개발하면서 이용한 AWS 클라우드를 활용하여 간단한 딥러닝 모델 학습부터 서버리스 API 구축까지에 대한 이야기를 해보려고 합니다. 전체적인 내용의 파트를 나누자면, 아래와 같이 크게 2가지 파트 - `딥러닝 모델 개발과정`과 `Serverless API 개발과정`으로 나눌 수 있을 것 같습니다.

 1. Amazon SageMaker에서 PyTorch를 활용한 딥러닝 이미지 분류 모델 학습 및 배포
 2. Django Rest Framework를 이용한 RESTful API 개발 & Zappa를 이용해 AWS Lambda에 Django 올리기


## 1. Amazon SageMaker에서 PyTorch를 활용한 딥러닝 이미지 분류 모델 학습 및 배포

우선 간단한 이미지 분류기를 만들기 위해서 학습에 사용할 데이터가 필요합니다. 이번 튜토리얼에서는 요즘에 핫(?)한 딥러닝 프레임워크인 PyTorch를 사용하여 모델 학습을 해보려고 하는데, PyTorch에는 Computer Vision 분야를 위한 [torchvision](https://pytorch.org/docs/stable/torchvision/index.html#torchvision) 이라는 패키지가 만들어져 있습니다. 여기에는 ImageNet이나 CIFAR10, MNIST 등과 같이 널리 사용되는 유명한 데이터셋(datasets)과 모델 아키텍처(models) 및 이미지용 데이터 변환기(transforms)가 포함되어 있습니다. 이 패키지를 사용하면 편리하게 대량의 이미지 데이터를 불러오고, 데이터 변환도 손쉽게 할 수 있습니다. 여기서는 **나만의 데이터셋**으로 세 종류의 고양이를 분류하는 모델을 만들기 위해서 `torchvision`의 `ImageFolder` 모듈을 사용합니다. 
...
<img src="/assets/12_DL image classification/trainset.jpg" alt="trainset">


### Data를 Amazon S3에 업로드하기
...
[/assets/12_DL image classification/classifier.py](classifier.py)
...
### PyTorch Predictor 배포하기 (Endpoint 생성)
...
```
<img src="/assets/12_DL image classification/inference.jpg" alt="inference">
...
<img src="/assets/12_DL image classification/inference-TL.jpg" alt="inference-TL">
